...
No joke, it's as simple as it gets. This formula usually gives the soil's moisture value with a 10\% margin, which is not fabulous, but still quite good for the price, and good enough for our application. Anyway, we did mention the sensor was inaccurate, and we'll compensate for that once we are in the field with a second field calibration.
...
## Water the plants
Now, remember when we talked about the Field Capacity, Permanent wilting point and Maximum Allowable Depletion levels? These will be your triggers to open and close the valve, possibly with a condition to only water between sunrise and sunset. Roughly speaking, the permanent wilting point is around half the field capacity, so we can simplify the irrigation rules to:

- Open the valve when **moisture = field capacity * (100\% - MAD/2)**
...
We can see that the excess water is drained by gravity overnight, the soil moisture stabilizing around 64\%. Is this an accurate value? No! But it is worth nothing that this was a *pure clay* soil.

Finally, to set the "open valve" level, we use MAD/2. So if we have cucumbers with a MAD of 20%, we will set the valve to open when the moisture level reaches **64\% * (100\% - 20\%/2) = 64\% * 90\% = 58\%**.
